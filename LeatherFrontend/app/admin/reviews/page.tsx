...
export default function AdminReviewsPage() {
  ...
  const fetchReviews = async () => {
    try {
      ...
      const resApproved = await apiFetch('/api/v1/admin/reviews/approved')
      if (resApproved?.data) setApprovedReviews(resApproved.data)
    } catch (e) {
      console.error(e)
    } finally {
      setLoading(false)
  ...
  const deleteApprovedReview = async (id: string) => {
    ...
    if (!confirm('Are you sure you want to delete this approved review?')) return
    try {
      const response = await apiFetch(`/api/v1/reviews/${id}`, { method: 'DELETE' })
      if (response && response.success !== false) {
        setApprovedReviews(prev => prev.filter(r => r._id !== id))
        alert('Approved review deleted successfully')
      } else {
        throw new Error(response?.message || 'Delete failed')
      }
    } catch (e: any) {
      console.error('Delete approved review error:', e)
      alert(e?.message || 'Failed to delete approved review')
    }
  }
  ...
  return (
    <div className="space-y-8">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative w-8 h-8 rounded-full overflow-hidden bg-gray-200">
                          {review.user?.profileImage && (
                            <Image src={cloudinaryOptimize(review.user.profileImage, 96) || review.user.profileImage} alt={review.user.userName} fill sizes="100vw" className="object-cover" />
                          )}
                        </div>
                        ...
                        <div className="flex gap-1 mt-2">
                          {review.imageUrls.map((url, i) => (
                            <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="block relative w-10 h-10 border rounded overflow-hidden">
                              <Image src={cloudinaryOptimize(url, 200) || url} alt="Review attachment" fill sizes="100vw" className="object-cover" />
                            </a>
                          ))}
                        </div>
                      )}
      ...
      {/* Approved Reviews */}
      <div>
        <h2 className="text-2xl font-serif">Approved Reviews</h2>
                  ...
                  <tr key={review._id} className="hover:bg-muted/50">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="relative w-8 h-8 rounded-full overflow-hidden bg-gray-200">
                          {review.user?.profileImage && (
                            <Image src={review.user.profileImage} alt={review.user.userName} fill sizes="100vw" className="object-cover" />
                          )}
                        </div>
                        <span className="font-medium">{review.user?.userName || 'Unknown User'}</span>
                      </div>
